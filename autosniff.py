#!/usr/bin/python2
#
# Beagle MiTM Author: @jkadijk
# Scotty MiTM Author: @sodaphish
# 
# Base decoderthread layout from the Impacket examples.
#
# enhancements: 
#    * Subnet class to handle all bitmasks or be replaced
#    * handle global configuration file
#    * add unit-testing
#    * handle command-line switches to specify some or all config params
#    * integrate SPLib and split-out classes into 
#    * use randomized IP for the bridge address
#
...
try:
    """
    import SPlib components we need
    """
    from sp.base.Config import Config
    from sp.base.Logging import *
    from sp.base import Exceptions
except ImportError as e:
    print( "missing sp python package: %s" % (e))
    sys.exit(2)


if os.getuid() != 0:
    """
    we have to run this script as root because we're going to be mangling all 
    sorts of system-level things that will fail if our UID > 0
    """
...
try:
    """
    catch any scenarios where we're missing packages 
    """
    import pcapy
    ...
    print( "missing pcapy or impacket python package: %s" % (e) )
    sys.exit(2)
...
class Netfilter():
    ...
    def updatetables(self):
        ...
        f=os.popen("brctl showmacs %s | grep %s | awk '{print $1}'" % (self.bridgeinterface, self.subnet.get_gatewaymac()))            
...
class Bridge():
    """
    so, now we're using system() and calling it a class?  fix this.
    """
    subnet = None
    bridgename = None
...
#    os.system("/usr/local/bin/hostapd -B /etc/hostapd/hostapd.conf")
    ...
    try:
        while(1):
            #TODO: make this so it creates a safe file!!!
